/**
 * Re-runs the notebook's inferential tests against the built data payload and
 * asserts the numbers still match EDA_AirBNB.ipynb's SciPy output.
 *
 * Plain .mjs on purpose: this runs inside `npm run build`, and Vercel's build
 * image picks its own Node minor. Depending on native TypeScript type stripping
 * (Node >= 22.18) would make the deploy fail on any older image, so the types
 * are stripped explicitly by esbuild instead.
 *
 * Usage: node scripts/verify-stats.mjs
 */
import { readFileSync } from 'node:fs';
import { transformSync } from 'esbuild';

/* ------------------------------------------------------------------ */
/* Load the app's own stats module                                     */
/*                                                                     */
/* src/lib/stats.ts is the single source of truth -- the five sections  */
/* under src/ import the same functions. Re-implementing them here      */
/* would let the dashboard drift from the notebook silently, which is   */
/* exactly what this script exists to prevent. So transpile it and      */
/* import the result as a data: URL, no temp file needed.               */
/* ------------------------------------------------------------------ */

const statsSource = readFileSync(
  new URL('../src/lib/stats.ts', import.meta.url),
  'utf8',
);

// A data: URL module has no base path, so relative imports inside stats.ts
// would resolve to nothing. It is self-contained today; fail loudly if not.
if (/^\s*import\s|^\s*export\s+\*?\s*(\{[^}]*\}\s*)?from\s/m.test(statsSource)) {
  throw new Error(
    'src/lib/stats.ts now has module imports, which the data: URL trick in ' +
      'this script cannot resolve. Bundle it to a temp file instead.',
  );
}

const { code } = transformSync(statsSource, {
  loader: 'ts',
  format: 'esm',
  target: 'node22',
});

const { anova, chiSquare, spearman } = await import(
...
const raw = JSON.parse(
  readFileSync(new URL('../src/data/listings.json', import.meta.url), 'utf8'),
);
const R = raw.rows;
const col = (i) => R.map((r) => r[i]);
...
const seg = (p) => (p < 500 ? 0 : p < 1000 ? 1 : 2);
...
let allOk = true;
const t = (n, g, w, tol) => { allOk = check(n, g, w, tol) && allOk; };
